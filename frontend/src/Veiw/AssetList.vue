...
const newAsset = ref({
  ...
  location: '', purchase_date: '', purchase_value: '',
  asset_code: '', purchase_order_id: ''
})

async function submitAsset() {
  if (!newAsset.value.name || !newAsset.value.asset_code || !newAsset.value.purchase_value) {
    alert('Name, Asset Code and Purchase Value are required'); return
  }
  await store.createAsset({
    ...newAsset.value,
    purchase_value: parseFloat(newAsset.value.purchase_value)
  })
  newAsset.value = { name:'', category:'', serial_number:'', location:'', purchase_date:'', purchase_value:'', asset_code:'', purchase_order_id:'' }
